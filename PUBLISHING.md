...
内容现在是 **每天一个 Markdown 文件**，跟你公众号的发布习惯一致。
不再有手写 HTML 字符串，**也不再有封面图**——本站定位是给人和 AI 阅读的纯文本简报。
